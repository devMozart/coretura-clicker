...
> **Status.** This is the original brief, kept for reference. The parts still worth reading
> before building are 3.7 (Funding Rounds) and 3.9 (news ticker copy) — neither is built yet.
> The rest shipped and then some: 17 producers rather than 12, ~104 upgrades, 22 achievements,
> 10 event types, plus a tabbed store, an achievements panel, and SEO/PWA work not described
> here at all. Where this document and the code disagree the code wins, and `src/content.ts`
> is the source of truth for every number — the tables below were not kept in step. Sections 6
> and 7 are historical: most of the roadmap is done and most of the open questions are settled.
...
**Reveal rule:** dropped. The full ladder is always listed so players can see what they are
working towards; unaffordable rows dim and the list scrolls.
...
- **Corners:** square, everywhere — panels, tooltips, toasts, event pickups, chips, buttons
  and rows all sit at `border-radius: 0`. The only rounded shapes are the genuinely circular
  ones: the brand mark, the Core and its pulse ring.
- **Layout:** top bar (brand + menu) · center stage (headline number, Core, hint, credit) ·
  right store (tabbed upgrades panel — Store / Active — then the producer list with
  ×1/×10/×100 buy toggle, then achievements). Stack to single column under ~820px. The news
  ticker in 3.9 is not built.
